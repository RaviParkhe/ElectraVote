...
public class ResultPage extends VBox {
    ...
    private HBox buildHeaderSection() {
        ...
        Label subtitle = new Label("Certified final results, position-wise breakdown analytics, and secure report generation");
        ...
        resultStatus.setStyle(FONT + "-fx-background-color: #ecfdf5; -fx-text-fill: #047857; -fx-font-weight: 900; -fx-font-size: 11.5px; -fx-padding: 6 14; -fx-background-radius: 20; -fx-border-color: #10b981; -fx-border-radius: 20;");
    ...
    private HBox buildElectionActionBar() {
        ...
        electionBar.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 12; -fx-border-color: #818cf8; -fx-border-radius: 12; -fx-border-width: 1.5; -fx-effect: dropshadow(gaussian, rgba(99,102,241,0.08), 8, 0, 0, 3);");
        ...
        electionCombo.setStyle(FONT + "-fx-background-color: #f8fafc; -fx-border-color: #cbd5e1; -fx-border-radius: 8; -fx-background-radius: 8; -fx-font-size: 13px; -fx-font-weight: 600; -fx-padding: 4 8;");
        ...
        Button exportReportBtn = createActionButton("📥 Export Certified Tally (.txt)", "#ecfdf5", e -> exportResultReport());
        
        exportReportBtn.setStyle(FONT + "-fx-background-color: #ecfdf5; -fx-text-fill: #047857; -fx-border-color: #10b981; -fx-border-radius: 8; -fx-font-weight: 800; -fx-font-size: 12.5px; -fx-padding: 8 16; -fx-background-radius: 8; -fx-cursor: hand;");

        electionBar.getChildren().addAll(selectLabel, electionCombo, barSpace, summarizeButton, exportReportBtn);
        return electionBar;
    }

    private Button createActionButton(String text, String colorHex, javafx.event.EventHandler<javafx.event.ActionEvent> handler) {
        Button btn = new Button(text);
        btn.setMinWidth(Region.USE_PREF_SIZE);
        btn.setStyle(FONT + "-fx-background-color: " + colorHex + "; -fx-text-fill: white; -fx-font-weight: 800; -fx-font-size: 12.5px; -fx-padding: 8 16; -fx-background-radius: 8; -fx-cursor: hand;");
    ...
    private void showSelectElectionMessage() {
        ...
        message.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 14; -fx-border-color: #cbd5e1; -fx-border-radius: 14; -fx-border-width: 1.5;");
        ...
        Label text = new Label("Select a certified election from the dropdown above to examine detailed position metrics and vote distributions.");
    ...
    private void exportResultReport() {
        ...
        if (selected == null) {
            showStyledAlert("Select Election", "Please select an election before exporting the certified report.", "⚠️", "#d97706", "#fffbeb");
            return;
        }
        ...
        if (file != null) {
            try (FileWriter writer = new FileWriter(file)) {
                ...
                writer.write("Certification Timestamp : " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd MMM yyyy, HH:mm:ss")) + "\n");
                ...
                for (PositionResult pos : result.positions) {
                    ...
                    for (CandidateResult c : pos.candidates) {
                        writer.write(String.format("  #%d %-20s : %4d votes (%.1f%%)\n", r, c.name, c.votes, c.percentage));
                        r++;
                    }
                    writer.write("\n");
                }
                ...
                showStyledAlert("Export Successful", "Certified election result report successfully saved to disk:\n" + file.getAbsolutePath(), "✓", "#16a34a", "#dcfce7");
            } catch (IOException e) {
    ...
    private void showSummary() {
        ...
        if (selectedElection == null) {
            showStyledAlert("Select Election", "Please select an election before generating the summary report.", "⚠️", "#d97706", "#fffbeb");
            return;
        }
        ...
        dialog.getDialogPane().setStyle("-fx-background-color: #ffffff; -fx-border-color: #818cf8; -fx-border-width: 1.5; -fx-border-radius: 14; -fx-background-radius: 14;");
        ...
        if (closeBtn != null) {
            closeBtn.setStyle(FONT + "-fx-background-color: #2563eb; -fx-text-fill: white; -fx-font-weight: 900; -fx-font-size: 13px; -fx-background-radius: 8; -fx-padding: 9 20; -fx-cursor: hand;");
        }
    ...
    private VBox createWinnerSummary(PositionResult position, CandidateResult winner) {
        ...
        card.setStyle("-fx-background-color: #f8fafc; -fx-background-radius: 12; -fx-border-color: #cbd5e1; -fx-border-radius: 12; -fx-border-width: 1.2;");
    ...
    private VBox createPositionResult(PositionResult position) {
        ...
        positionBox.setStyle("-fx-background-color: #ffffff; -fx-background-radius: 14; -fx-border-color: #cbd5e1; -fx-border-radius: 14; -fx-border-width: 1.5; -fx-effect: dropshadow(gaussian, rgba(15,23,42,0.04), 8, 0, 0, 3);");
    ...
    private HBox createCandidateResultCard(CandidateResult candidate, int rank, boolean isWinner) {
        ...
        if (isWinner) {
            card.setStyle("-fx-background-color: linear-gradient(to right, #fefce8 0%, #fef9c3 100%); -fx-background-radius: 10; -fx-border-color: #facc15; -fx-border-radius: 10; -fx-border-width: 1.5; -fx-effect: dropshadow(gaussian, rgba(234,179,8,0.15), 6, 0, 0, 2);");
        } else {
            card.setStyle("-fx-background-color: #f8fafc; -fx-background-radius: 10; -fx-border-color: #e2e8f0; -fx-border-radius: 10; -fx-border-width: 1.2;");
        }

        Label rankLabel = new Label("#" + rank);
        rankLabel.setMinWidth(32);
        rankLabel.setStyle(FONT + "-fx-font-size: 15px; -fx-font-weight: 900; -fx-text-fill: " + (isWinner ? "#ca8a04" : "#64748b") + ";");
        ...
        avatar.setStyle(FONT + "-fx-background-color: " + (isWinner ? "#fef08a" : "#eff6ff") + "; -fx-text-fill: " + (isWinner ? "#854d0e" : "#1d4ed8") + "; -fx-font-size: 15px; -fx-font-weight: 900; -fx-background-radius: 21; -fx-border-color: " + (isWinner ? "#fde047" : "#bfdbfe") + "; -fx-border-radius: 21;");
        ...
        if (isWinner) {
            ...
            winnerBadge.setStyle(FONT + "-fx-background-color: #fef08a; -fx-text-fill: #854d0e; -fx-font-size: 9.5px; -fx-font-weight: 900; -fx-padding: 2 8; -fx-background-radius: 10; -fx-border-color: #facc15; -fx-border-radius: 10;");
        ...
        Label tallyReadout = new Label(candidate.votes + " ballots (" + String.format("%.1f%%", candidate.percentage) + ")");
        tallyReadout.setStyle(FONT + "-fx-font-size: 12px; -fx-font-weight: 800; -fx-text-fill: " + (isWinner ? "#047857" : "#1e293b") + ";");
        ...
        voteBarFill.setStyle("-fx-background-color: linear-gradient(to right, " + (isWinner ? "#10b981, #059669" : "#3b82f6, #2563eb") + "); -fx-background-radius: 7;");
